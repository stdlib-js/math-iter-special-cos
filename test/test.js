...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterCos, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterCos( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the cosine of each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 100;
	it = iterCos( linspace( 0.0, 6.14, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 6.14, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = cos( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the cosine of each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
